...
AlipaySubmit.prototype.query_timestamp = function (callback) {
	...
	console.log('query_timestamp -- ' + url);
	require('https').get(url, function(res) {
        console.log('query_timestamp get response ' + res);
		var responseText='';
        res.on('data', function(chunk){
            console.log("query_timestamp on data");
            responseText += chunk;
        });
        res.on('end', function(){
            console.log("query_timestamp on end");
			var doc = new DOMParser().parseFromString(responseText);
			var itemEncrypt_key = doc.getElementsByTagName( "encrypt_key" );
			var encrypt_key = itemEncrypt_key[0].childNodes[0].nodeValue;
