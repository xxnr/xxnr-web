...
function json_agent_info_get(id){
	...
	UserService.get({userid:id}, function(err, user){
		...
		PotentialCustomerService.getStatistic(user._id, function(err, totalCount, registeredCount){
			if(err){
				self.respond({code:1001, message:'获取新农经纪人信息失败'});
				return;
			}

			self.respond({code:1000, agent:{name:user.name, phone: user.account, address:user.address, totalCount:totalCount, registeredCount:registeredCount}});
		})
	})
}